...
if __name__ == "__main__":
    ...
    queries_to_rewrite = [query for query, score in zip(original_queries, well_formed_scores) if score < selected_threshold]
    print(f"We have {len(queries_to_rewrite)} to Rewrite:")
    ...
    for query in queries_to_rewrite:
        response = query_rewriter.query(
            query)  # Get the response from the model
        ...
        if response and isinstance(response, list) and 'generated_text' in \
            ...
            rewritten_queries.append(
                {'generated_text': generated_text})  # Append as a dictionary

    # Now that we have the correctly formatted rewritten queries, clean them
    cleaned_queries = preprocess_rewritten_queries(rewritten_queries)

    # Print out the cleaned queries
